...
> Code-side complete 2026-07-19 (H-220..H-224 + H-225's code half). Postgres
> via SQLAlchemy behind DATABASE_URL (refuses to start without it; Docker
> Postgres locally on port 5433, service container in CI, Neon in prod);
> accounts/tokens/packages/versions are DB rows; archives on disk locally /
> R2 in prod (env-selected backend); streaming downloads both sides; per-IP
> rate limits; /search + `xelian search` + website search on one endpoint;
> XELIAN_TOKEN/XELIAN_USERNAME env auth; XELIAN_REGISTRY_URL beats stored
> creds; registry Dockerfile + render.yaml committed;
> XELIAN_DEFAULT_REGISTRY_URL bakes the prod URL into release binaries.
>
> **Remaining owner-only actions for H-225/H-226 (all free tier):**
> - [ ] Create a Neon project -> copy DATABASE_URL into Render
> - [ ] Create a Cloudflare R2 bucket + API token -> XELIAN_R2_* into Render
> - [ ] Deploy on Render via render.yaml (repo must be on GitHub first)
> - [ ] Point registry.<domain> at it (after H-200 domain claim)
> - [ ] UptimeRobot monitor on /health every 5 min (keep-warm)
> - [ ] Enable Neon + R2 backups; do one restore drill; add Sentry (H-226)

- [x] **H-220 — Data layer: Postgres via SQLAlchemy + `DATABASE_URL`**
...
- [x] **H-221 — Real accounts + persisted tokens**
...
- [x] **H-222 — Registry limits + streaming**
...
- [x] **H-223 — Non-interactive auth + URL precedence**
...
- [x] **H-224 — Search: registry endpoint + `xelian search`**
...
- [ ] **H-225 — Deploy the registry publicly (free tier)** *(code artifacts
  done — Dockerfile, render.yaml, bake-able default URL; the deploy itself
  is in the owner checklist above)*
...
> Head start 2026-07-19: 16 seed packages (8 agents, 8 MCP servers — pure
> stdlib, zero keys, all smoke-tested) live in `seed/`, generated by
> `scripts/make_seed.py` and published to any registry with
> `scripts/publish_seed.sh` under the official `xelian` namespace. Published
