...
//!
//! On the first run of a given `(name, version)`, Xelian prompts the user to
//! grant or deny each declared permission. The decision is persisted so
//! subsequent runs do not re-prompt. This is **disclosure-only** — Xelian does
//! not technically enforce permissions in V1 (§16.2, §20.4).
//!
//! Grant state is stored under `~/.xelian/permissions/local/<name>/<version>.toml`
//! (see [`crate::cache::XelianHome::local_grants_path`]) — deliberately *outside* the
...
//! terminal: for `package-type = "mcp"` the child inherits Xelian's
...
mod tests {
    ...
    fn no_permissions_skips_everything() {
        ...
        let home = XelianHome::at(dir.path());
