//! The `xelian push` validation + build pipeline (SPEC.md §8.1): an ordered,
//! fail-fast sequence of static checks culminating in a deterministic
//! `.xelian` archive. No step here ever executes package code or contacts
//! the network (§8.2, §8.4).
...
/// SPEC.md §5.3: files (beyond `xelian.toml`, checked separately as part of
...
pub enum ValidateError {
    /// Step 1: `xelian.toml` itself is malformed TOML / missing a required field.
    #[error(transparent)]
    ManifestParse(#[from] ManifestError),

    /// Step 1: `xelian.toml` parsed but failed a semantic rule (§6, §8.1).
    #[error(transparent)]
    ManifestValidation(#[from] ValidationError),

    /// Step 2: an existing `xelian.lock` on disk does not parse.
    #[error("existing xelian.lock is malformed: {0}")]
    ...
    /// Step 7: `xelian.lock` generation failed (e.g. a declared native
    /// lockfile is missing on disk).
    #[error("failed to generate xelian.lock: {0}")]
...
pub struct BuildOutcome {
    /// Path to the freshly built `.xelian` archive.
    pub archive_path: PathBuf,
    /// The `package-checksum` recorded in `xelian.lock` (SPEC.md §7.3).
    pub package_checksum: String,
    /// Non-fatal manifest warnings (SPEC.md §17) to surface to the user.
    pub warnings: Vec<ValidationWarning>,
    /// The lockfile written to `root/xelian.lock`.
...
/// (`<name>-<version>.xelian` written into `root`).
///
/// On success, `root/xelian.lock` has been (re)written and the archive at
...
pub fn validate_and_build(root: &Path, out: Option<&Path>) -> Result<BuildOutcome, ValidateError> {
    // --- Step 1: parse + validate xelian.toml ---
    let manifest_path = root.join("xelian.toml");
    ...
    // --- Step 2: re-validate an existing xelian.lock, if any ---
    let lock_path = root.join("xelian.lock");
    ...
    // --- Step 3: required files exist (xelian.toml already handled above) ---
    ...
    // --- Steps 6 + 7: compute package-checksum and generate xelian.lock ---
    ...
    // The freshly written xelian.lock MUST be in the archive (§5.3) even on
...
mod tests {
    ...
    /// A minimal, fully valid package: xelian.toml + README + LICENSE +
...
"#,
    ...
    fn manifest_error_surfaces_even_when_readme_is_also_missing() {
        ...
        write_file(dir.path(), "xelian.toml", "not valid toml {{{");
    ...
    fn empty_command_value_is_error() {
        let dir = tempdir().unwrap();
        scaffold_valid_package(dir.path());
        let toml = fs::read_to_string(dir.path().join("xelian.toml")).unwrap();
        let with_commands = format!("{toml}\n[commands]\ntest = \"   \"\n");
        write_file(dir.path(), "xelian.toml", &with_commands);
    ...
    fn malformed_existing_lockfile_is_distinct_error() {
        let dir = tempdir().unwrap();
        scaffold_valid_package(dir.path());
        write_file(dir.path(), "xelian.lock", "not valid toml {{{");
    ...
    fn archive_never_contains_gitignored_secret_or_itself() {
        ...
        assert!(names.contains(&"xelian.toml".to_string()));
        assert!(names.contains(&"xelian.lock".to_string()));
