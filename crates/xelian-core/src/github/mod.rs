...
pub fn infer_manifest(
    ...
    // Docker-only projects (no Python/Node package, just a Dockerfile) are
    // handled by a generated launcher that builds and runs the image — see
    // `generate_docker_manifest`. Detected here so it short-circuits before the
    // normal language flow.
    let language = match detect_language(checkout) {
        Ok(lang) => lang,
        Err(GithubError::DockerOnly { .. }) => {
            return generate_docker_manifest(checkout, repo);
        }
        Err(e) => return Err(e),
    };
    eprintln!("Detected language: {}", detect::language_label(language));

    let found = match entrypoint::infer(checkout, language, repo.package_basis()) {
        Some(f) => f,
        None => {
            // No runnable Python/Node entrypoint. A repo that also ships a
            // Dockerfile (e.g. OpenHands) runs via Docker instead.
            if checkout.join("Dockerfile").is_file()
                || checkout.join("docker-compose.yml").is_file()
            {
                return generate_docker_manifest(checkout, repo);
            }
            return Err(GithubError::NoEntrypoint {
...
/// Build a manifest for a Docker-only project: a generated Python launcher
/// (`xelian_launcher.py`) that runs `docker build` + `docker run`, plus an
/// empty requirements file so the Python runtime provisions but installs
/// nothing. This reuses the existing Python launch path — no new runtime — so
/// `xelian run`/`add` executes the container itself.
fn generate_docker_manifest(
    checkout: &Path,
    repo: &RepoRef,
) -> Result<InferredManifest, GithubError> {
    eprintln!("Detected a Docker project — will build and run it with Docker.");

    let name = derive_package_name(repo.package_basis());
    let image = format!("xelian-{name}");

    // The launcher builds the image from the package dir (which holds the
    // Dockerfile) and runs it with stdio inherited (agent REPL or MCP stdio).
    let launcher = format!(
        "# Generated by `xelian add` — this project runs via Docker.\n\
         import subprocess, sys\n\
         \n\
         subprocess.run([\"docker\", \"build\", \"-t\", \"{image}\", \".\"], check=True)\n\
         sys.exit(subprocess.run([\"docker\", \"run\", \"-i\", \"--rm\", \"{image}\"]).returncode)\n"
    );
    fs::write(checkout.join("xelian_launcher.py"), &launcher).map_err(|e| GithubError::Io {
        path: checkout.join("xelian_launcher.py"),
        source: e,
    })?;
    // Empty deps: the Python runtime creates a venv and installs nothing; all
    // the real dependencies live inside the Docker image.
    fs::write(
        checkout.join("xelian-docker.requirements.txt"),
        "# Docker project: no Python dependencies; the image carries them.\n",
    )
    .map_err(|e| GithubError::Io {
        path: checkout.join("xelian-docker.requirements.txt"),
        source: e,
    })?;

    let description = format!(
        "Imported from https://github.com/{}/{} (runs via Docker)",
        repo.owner, repo.repo
    );
    let toml = render_manifest_toml(
        &name,
        &description,
        PackageType::Agent,
        Language::Python,
    ...
    fs::write(checkout.join("xelian.toml"), &toml).map_err(|e| GithubError::Io {
        path: checkout.join("xelian.toml"),
        source: e,
    })?;
