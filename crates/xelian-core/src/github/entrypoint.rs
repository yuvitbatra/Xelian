...
pub struct Entrypoint {
    ...
    pub exists: bool,
    /// Source for a generated launcher, when the project's only declared way
    /// to run is a console script (`pkg:main`) with no runnable file of its
    /// own. Writing this file is the caller's job; see [`launcher_source`].
    pub shim: Option<String>,
}

impl Entrypoint {
    /// An entrypoint that is a real file already present in the checkout.
    fn found(path: impl Into<String>) -> Self {
        Self {
            path: path.into(),
            exists: true,
            shim: None,
    ...
    /// An entrypoint a build step will produce.
    fn from_build(path: impl Into<String>) -> Self {
        Self {
            path: path.into(),
            exists: false,
            shim: None,
...
/// The filename of the launcher Xelian generates for console-script projects.
pub const LAUNCHER_FILENAME: &str = "xelian_launcher.py";

/// Source for a launcher that calls a console-script target.
///
/// A large share of Python MCP servers declare only
/// `[project.scripts] name = "pkg:main"` and ship no `__main__.py` — there is
/// no file to execute, which is why these previously could not be imported.
/// pip solves this by generating a console script at install time; this is the
/// same idea, written into the package so the manifest's entrypoint stays a
/// real path.
fn launcher_source(module: &str, function: &str, target: &str) -> String {
    format!(
        "# Generated by `xelian add` — launcher for the console script `{target}`.\n\
         # This project declares no runnable module of its own, so Xelian calls its\n\
         # declared entry point directly, the way a pip-generated script does.\n\
         import importlib\n\
         import sys\n\
         \n\
         _entry = importlib.import_module(\"{module}\")\n\
         for _part in \"{function}\".split(\".\"):\n\
         \x20   _entry = getattr(_entry, _part)\n\
         \n\
...
fn infer_python(checkout: &Path, name_hint: &str) -> Option<Entrypoint> {
    ...
    for candidate in CANDIDATES {
        if checkout.join(candidate).is_file() {
            return Some(Entrypoint::found((*candidate).to_string()));
        }
    }
    ...
    for name in &declared_names {
        ...
        if checkout.join(&candidate).is_file() {
            return Some(Entrypoint::found(candidate));
        }
    }
...
fn resolve_python_target(checkout: &Path, target: &str) -> Option<Entrypoint> {
    ...
    for prefix in ["", "src/"] {
        ...
        if checkout.join(&as_module).is_file() {
            return Some(Entrypoint::found(as_module));
        }
        ...
        if checkout.join(&as_package).is_file() {
            return Some(Entrypoint::found(as_package));
        }
    }

    // The module is importable but exposes no runnable file — the project's
    // only declared way to run is the console script itself. Generate a
    // launcher that calls it, exactly as a pip-installed console script would.
    let function = target.split(':').nth(1)?.trim();
    if function.is_empty() || !is_python_identifier(function) {
        return None;
    }
    for prefix in ["", "src/"] {
        if checkout
            .join(format!("{prefix}{rel}/__init__.py"))
            .is_file()
        {
            return Some(Entrypoint {
                path: LAUNCHER_FILENAME.to_string(),
                exists: false,
                shim: Some(launcher_source(module_path, function, target)),
            });
        }
    }
    None
...
/// Whether `s` is a plain Python identifier, or a dotted attribute path of
/// them (`main`, `cli.app`). Guards against interpolating anything else into
/// generated source.
...
fn find_module_main(checkout: &Path, module: &str) -> Option<Entrypoint> {
    ...
    for prefix in ["", "src/"] {
        ...
        if checkout.join(&p).is_file() {
            return Some(Entrypoint::found(p));
        }
    }
    None
}
...
fn infer_node(checkout: &Path) -> Option<Entrypoint> {
    ...
    let accept = |path: String| -> Option<Entrypoint> {
        if is_typescript(&path) {
            return buildable.then(|| Entrypoint::from_build(compiled_output_for(&path)));
        }
        let exists = checkout.join(&path).is_file();
        if exists {
            Some(Entrypoint::found(path))
        } else if buildable && looks_like_build_output(&path) {
            Some(Entrypoint::from_build(path))
        } else {
            None
        }
    };
    ...
    if buildable {
        for src in ["src/index.ts", "src/main.ts", "index.ts"] {
            if checkout.join(src).is_file() {
                return Some(Entrypoint::from_build("dist/index.js".to_string()));
            }
        }
    }

    // 5. Published-package fallback: a `bin` with a proper package name means
    //    this is (almost always) published to npm and meant to be run with
    //    `npx -y <name>` — that is how MCP-server READMEs universally document
    //    it. Generate a launcher that does exactly that. This handles packages
    //    whose build can't run in isolation (e.g. pnpm-workspace subpackages)
    //    but which ARE installable from npm. Last resort, so a runnable source
    //    entrypoint always wins over pulling from npm.
    if pkg.bin.is_some() {
        if let Some(name) = pkg.name.as_deref() {
            if is_publishable_npm_name(name) {
                return Some(Entrypoint {
                    path: NODE_LAUNCHER_FILENAME.to_string(),
                    exists: false,
                    shim: Some(npx_launcher_source(name)),
                });
            }
        }
    }
...
/// The launcher Xelian generates to run a published npm package via `npx`.
pub const NODE_LAUNCHER_FILENAME: &str = "xelian_launcher.cjs";

/// Whether an npm package name is a real, publishable name (so `npx -y <name>`
/// can resolve it): a plain name or a `@scope/name`, from the npm charset.
fn is_publishable_npm_name(name: &str) -> bool {
    let core = name.strip_prefix('@').map_or(name, |rest| rest);
    !name.is_empty()
        && !name.starts_with('.')
        && core.chars().all(|c| {
...
/// A CommonJS launcher that runs `npx -y <package>` with the terminal's stdio
/// inherited (so it works for both an agent REPL and an MCP stdio server).
fn npx_launcher_source(package: &str) -> String {
    format!(
        "// Generated by `xelian add` — this project is published to npm and is\n\
         // run with `npx -y {package}` (as its README documents). Xelian runs it\n\
         // that way rather than building from source.\n\
         const {{ spawnSync }} = require(\"child_process\");\n\
         const r = spawnSync(\"npx\", [\"-y\", \"{package}\", ...process.argv.slice(2)], {{ stdio: \"inherit\" }});\n\
...
mod tests {
    ...
    #[test]
    fn console_script_only_package_gets_a_generated_launcher() {
        // mcp-atlassian shape: `[project.scripts] x = "pkg:main"`, and the
        // package exposes only __init__.py — no file to execute.
        let d = tempdir().unwrap();
        write(
            d.path(),
            "pyproject.toml",
        ...
        let e = infer(d.path(), Language::Python, "mcp-atlassian").unwrap();
        assert_eq!(e.path, LAUNCHER_FILENAME);
        let source = e.shim.expect("a launcher must be generated");
        assert!(source.contains("mcp_atlassian"), "source: {source}");
        assert!(source.contains("main"), "source: {source}");
        assert!(source.contains("sys.exit"), "source: {source}");
    }

    #[test]
    fn a_runnable_file_is_always_preferred_over_a_generated_launcher() {
        let d = tempdir().unwrap();
        write(
            d.path(),
            "pyproject.toml",
        ...
        write(d.path(), "pkg/__init__.py", "def main(): pass\n");
        write(d.path(), "pkg/__main__.py", "print(1)\n");

        let e = infer(d.path(), Language::Python, "pkg").unwrap();
        assert_eq!(e.path, "pkg/__main__.py");
        assert!(e.shim.is_none());
    }

    #[test]
    fn a_malformed_console_target_does_not_generate_a_launcher() {
        // Nothing that is not a plain identifier may reach generated source.
        let d = tempdir().unwrap();
        write(
            d.path(),
            "pyproject.toml",
            "[project]\nname = \"pkg\"\n[project.scripts]\n\
             x = \"pkg:main(); import os\"\n",
        );
        write(d.path(), "pkg/__init__.py", "");

        assert_eq!(infer(d.path(), Language::Python, "pkg"), None);
    }
    ...
    #[test]
    fn a_typescript_main_without_a_build_falls_back_to_npx() {
        // sentry-mcp-stdio shape: `main: index.ts`, no build script. Node can't
        // run a .ts file, but the package has a `bin` + a publishable name, so
        // it is (almost certainly) on npm and runs via `npx -y <name>`. We
        // generate an npx launcher rather than giving up.
        let d = tempdir().unwrap();
        write(
            d.path(),
            "package.json",
            r#"{"name":"sentry-mcp-stdio","main":"index.ts","bin":{"s":"./index.ts"}}"#,
        );
        write(d.path(), "index.ts", "export {}\n");

        let e = infer(d.path(), Language::Node, "sentry-mcp-stdio").unwrap();
        assert_eq!(e.path, NODE_LAUNCHER_FILENAME);
        let src = e.shim.expect("npx launcher generated");
        assert!(src.contains("npx"), "src: {src}");
        assert!(src.contains("sentry-mcp-stdio"), "src: {src}");
    }

    #[test]
    fn a_local_only_package_name_does_not_trigger_npx() {
        // No `bin` → no npx fallback (nothing says it's a runnable npm CLI).
        let d = tempdir().unwrap();
        write(
            d.path(),
            "package.json",
            r#"{"name":"just-a-lib","dependencies":{"x":"^1"}}"#,
        );
        assert_eq!(infer(d.path(), Language::Node, "just-a-lib"), None);
    }
