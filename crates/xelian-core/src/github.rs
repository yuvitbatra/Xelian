...
//! that SHA, detecting its language, inferring a `xelian.toml`, and building
//! the `.xelian` package + `xelian.lock`. It performs no publishing and no
//! registry interaction (§12.3) — running the built package (§12.2 step 7)
//! and wiring this into the `xelian add` CLI command are later work.
//!
...
pub enum GithubError {
    ...
    /// `git` or `curl` is required for `xelian add` and was not found (or is
    ...
    /// The generated (or pre-existing) `xelian.toml` failed to parse.
    #[error(transparent)]
    ...
    /// `xelian.lock` generation or serialization failed (§7).
    #[error(transparent)]
    Lockfile(#[from] LockfileError),

    /// The repository's own `.gitignore` would exclude `xelian.toml` from
...
/// values Xelian renders (repo-derived text and version constraints).
fn toml_escape(s: &str) -> String {
    s.replace('\\', "\\\\").replace('"', "\\\"")
}

/// Render the inferred `xelian.toml` contents (SPEC.md §12.2 step 3).
...
/// Infer and write a `xelian.toml` manifest into `checkout` (SPEC.md §12.2
...
///
/// If `checkout` already contains a `xelian.toml` (the repository is already
/// a Xelian package), it is kept verbatim and returned as-is — no
...
/// Generate `xelian.lock` and build the `.xelian` archive for an imported
...
/// `xelian push` (§12.3) re-runs full validation. Instead this mirrors
...
pub fn build_import(checkout: &Path) -> Result<(), GithubError> {
    ...
    // Force-include the freshly written xelian.lock, exactly as
...
pub struct ImportOutcome {
    ...
    /// cached directory already contained `xelian.toml` and `xelian.lock`),
    /// so detection/inference/lock/archive were all skipped.
    pub from_cache: bool,
}

/// Import a GitHub repository as a local Xelian package (SPEC.md §12.2 steps
...
/// language, infer a `xelian.toml`, and build `xelian.lock` + the `.xelian`
/// archive. Does not publish anything (§12.3) and does not run the package —
/// running it (§12.2 step 7) is the caller's responsibility.
pub fn import_github(url: &str, home: &XelianHome) -> Result<ImportOutcome, GithubError> {
    ...
    // fetch itself rather than on xelian.toml/xelian.lock presence: a
    // freshly downloaded repo may *ship* those files (it is already a Xelian
    // package), but its committed xelian.lock can be stale relative to the
    // checkout, so a fresh import must still regenerate the lock and archive
    // (§12.2 steps 4–5) — `infer_manifest` keeps a shipped xelian.toml
    // verbatim either way.
    if was_cached {
...
mod tests {
    ...
    fn python_fixture_infers_runtime_and_entrypoint() {
        ...
        assert!(dir.path().join("xelian.toml").is_file());
    }
    ...
    fn build_import_errors_clearly_when_gitignore_excludes_xelian_toml() {
        ...
        // A hostile/incidental .gitignore that would swallow xelian.toml too.
