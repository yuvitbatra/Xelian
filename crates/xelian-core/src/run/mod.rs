//! The `xelian run` local-archive preparation pipeline (SPEC.md §9): checksum
//! verification, safe extraction, and re-validation of a `.xelian` archive.
//!
//! V1 scope (decision 2026-07-16): this module only prepares a package from a
//! LOCAL `.xelian` archive path — the registry-ref and GitHub-URL target
...
/// The three target forms that `xelian run` accepts (SPEC.md §9.2).
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum RunTarget {
    /// A local `.xelian` archive path.
...
/// Parse a `xelian run` target string into one of the three accepted forms
...
pub fn parse_run_target(target: &str) -> Result<RunTarget, RunError> {
    ...
    // Local archive: ends with .xelian or is an existing file
    let path = Path::new(target);
    if target.ends_with(".xelian") || path.is_file() {
...
pub enum RunError {
    ...
    /// The archive has no `xelian.lock` entry at all.
    #[error(
        "archive {path} does not contain a xelian.lock — this is not a valid Xelian package",
        path = path.display()
    )]
    MissingLockfile { path: PathBuf },

    /// The archive's `xelian.lock` entry exists but fails to parse.
    #[error("xelian.lock inside the archive is malformed: {0}")]
    LockfileParse(#[source] LockfileError),

    /// The archive's `xelian.lock` has no `package-checksum` recorded, so
    /// integrity cannot be verified (SPEC.md §9.4).
    #[error(
        "xelian.lock inside the archive has no package-checksum recorded; \
         cannot verify package integrity"
    )]
    MissingPackageChecksum,

    /// The recomputed package-checksum does not match the one recorded in
    /// the archive's own `xelian.lock` (SPEC.md §9.4). The archive is
    /// aborted before any extraction happens.
    #[error(
        "checksum mismatch: recomputed package-checksum ({actual}) does not match \
         the package-checksum recorded in xelian.lock ({expected}) — the archive \
         may be corrupt or tampered with; aborting before extraction"
    )]
    ChecksumMismatch { expected: String, actual: String },

    /// The archive has no `xelian.toml` entry at all.
    #[error(
        "archive {path} does not contain a xelian.toml — this is not a valid Xelian package",
        path = path.display()
    )]
    MissingManifest { path: PathBuf },

    /// `xelian.toml` (from the archive, or later from the extracted
    /// directory) fails to parse.
    #[error(transparent)]
    ManifestParse(#[from] ManifestError),

    /// `xelian.toml` parses but fails semantic validation (SPEC.md §9.6).
    #[error(transparent)]
    ...
    #[error(
        "internal error: computed package destination {dest} is not confined to {packages_root}; \
         refusing to extract outside the Xelian packages directory",
        dest = dest.display(),
...
/// The outcome of successfully preparing a local `.xelian` archive: it has
...
/// 2. Recompute the package-checksum and compare against `xelian.lock`'s
///    recorded value; mismatch aborts before any extraction or writes.
/// 3. Parse `xelian.toml` (from the archive) to determine `(name, version)`
...
/// 5. Re-parse and re-validate `xelian.toml` from the extracted directory,
///    and check OS compatibility (§9.6, §9.6.1).
pub fn run_local_archive(archive: &Path, home: &XelianHome) -> Result<Prepared, RunError> {
    ...
    // --- Parse xelian.toml (pre-extraction) purely for cache addressing. ---
    ...
    if !from_cache {
        // A `true` result means a concurrent `xelian run` of this same
...
/// Run the preparation pipeline for a `.xelian` archive that was downloaded
...
pub fn run_registry_archive(
    ...
    // --- Parse xelian.toml for cache addressing ---
    let (_, manifest_bytes, _) = entries
        .iter()
        .find(|(p, _, _)| p == "xelian.toml")
        .ok_or_else(|| RunError::MissingManifest {
...
/// Re-parse and re-validate `xelian.toml` from an already-extracted package
...
pub fn prepare_environment(
    ...
    if !env_dir.join("xelian-env.ok").is_file() {
        manager
...
/// Read every regular-file entry out of a gzip-compressed tar (`.xelian`)
...
mod tests {
    ...
    fn parse_local_archive_by_extension() {
        let result = parse_run_target("./my-agent.xelian").unwrap();
        assert_eq!(
            result,
            RunTarget::LocalArchive(PathBuf::from("./my-agent.xelian"))
        );
    }
...
"#
    ...
    fn lockfile_with_checksum(version: &str, checksum: Option<String>) -> Lockfile {
        Lockfile {
            spec_version: 1,
            xelian_version: crate::XELIAN_VERSION.to_string(),
    ...
    fn sample_entries(name: &str, version: &str, os: &[&str]) -> Vec<(String, Vec<u8>)> {
        ...
        let mut files: Vec<(String, Vec<u8>)> = vec![
            ("xelian.toml".to_string(), manifest_toml.into_bytes()),
    ...
    /// nothing stops a hand-crafted `.xelian` file from having one.
    ...
    fn happy_path_prepares_and_extracts_the_package() {
        ...
        let archive_path = dir.path().join("sample-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);

        let home = XelianHome::at(dir.path().join("home"));
        ...
        assert!(prepared.package_dir.join("xelian.toml").is_file());
        assert!(prepared.package_dir.join("src/main.py").is_file());
        assert!(prepared.package_dir.join("xelian.lock").is_file());
    }
    ...
    fn checksum_mismatch_aborts_before_extraction() {
        ...
        // Mutate a non-xelian.lock file's contents after the checksum was
        ...
        let archive_path = dir.path().join("bad-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn missing_lockfile_is_a_clear_error() {
        ...
        entries.retain(|(p, _)| p != "xelian.lock");
        let archive_path = dir.path().join("no-lock-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn missing_package_checksum_is_a_clear_error() {
        ...
        let archive_path = dir.path().join("no-checksum-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn malformed_lockfile_is_a_clear_error() {
        ...
        let archive_path = dir.path().join("bad-lock-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn archive_with_traversal_entry_is_rejected_and_nothing_is_extracted() {
        ...
        let archive_path = dir.path().join("evil-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn manifest_name_path_traversal_is_rejected_before_any_extraction() {
        ...
        let archive_path = dir.path().join("evil-name.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn manifest_version_path_traversal_is_rejected_before_any_extraction() {
        ...
        let archive_path = dir.path().join("evil-version.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn skip_extraction_when_destination_already_cached() {
        ...
        let archive_path = dir.path().join("cached-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn run_fails_when_current_os_is_not_in_the_declared_list() {
        ...
        let archive_path = dir.path().join("os-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    fn run_succeeds_when_current_os_is_in_the_declared_list() {
        ...
        let archive_path = dir.path().join("os-ok-agent-1.0.0.xelian");
        write_archive(&archive_path, &entries);
    ...
    #[test]
    fn validate_extracted_rejects_missing_xelian_toml() {
        let dir = tempdir().unwrap();
        let err = validate_extracted(dir.path()).unwrap_err();
    ...
    fn executable_bit_is_preserved_through_the_real_build_and_run_pipeline() {
        ...
        // xelian.lock, with a package-checksum computed over everything
        // except itself, exactly as `xelian push` would produce it.
        ...
        let lock = lockfile_with_checksum("1.0.0", Some(checksum));
        fs::write(pkg_root.join("xelian.lock"), lock.to_toml_string().unwrap()).unwrap();

        let files = crate::package::collect_files(&pkg_root).unwrap();
        let archive_path = dir.path().join("exec-agent-1.0.0.xelian");
