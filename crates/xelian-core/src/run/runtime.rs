...
/// `(name, version)` env dir. `mkdir` is atomic on every platform Xelian
...
/// error after a timeout so a crashed holder cannot wedge Xelian forever.
struct EnvLock {
    path: PathBuf,
}
            ...
            match std::fs::create_dir(&lock_path) {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                    if start.elapsed() > Self::TIMEOUT {
                        return Err(RuntimeError::Process(format!(
                            "timed out waiting for another xelian process to finish \
                             installing this environment (lock: {}); if no other \
                             xelian is running, delete that directory and retry",
                            lock_path.display()
                        )));
                    }
...
impl PythonRuntimeManager {
    /// Search for `uv` in `PATH` or at `~/.xelian/runtimes/bin/uv`.
    fn find_uv(&self, home: &XelianHome) -> Option<PathBuf> {
...
impl RuntimeManager for PythonRuntimeManager {
    ...
    fn install_dependencies(
        ...
        if install_result.is_err() {
            ...
            if env_dir.exists() && !env_dir.join("xelian-env.ok").is_file() {
...
impl NodeRuntimeManager {
    ...
    fn find_cached_node(&self, home: &XelianHome, req: &semver::VersionReq) -> Option<PathBuf> {
    ...
    fn download_node(&self, home: &XelianHome, version: &str) -> Result<PathBuf, RuntimeError> {
...
mod tests {
    ...
    fn test_extensible_dispatch_with_mock_manager() {
        let home = XelianHome::at("/tmp/mock-home");
