//! File-set collection and deterministic `.xelian` (tar.gz) archive building
...
/// `false` on non-Unix platforms (Xelian archives built there simply never
...
/// files to include in a `.xelian` archive as
...
/// (`*.xelian`) are always excluded so a rebuild never ingests a prior
...
pub fn collect_files(root: &Path) -> Result<Vec<(String, PathBuf)>, PackageError> {
    ...
    for entry in builder.build() {
        ...
        if archive_path.ends_with(".xelian") || archive_path.ends_with(".xelian.tmp") {
            continue;
        }
...
/// Build a deterministic `.xelian` archive (gzip-compressed tar, SPEC.md
...
mod tests {
    ...
    #[test]
    fn previous_xelian_archive_is_excluded_from_collection() {
        let dir = tempdir().unwrap();
        write_file(dir.path(), "README.md", b"# hi");
        write_file(dir.path(), "stale-1.0.0.xelian", b"not a real archive");
    ...
    fn identical_inputs_produce_byte_identical_archives() {
        ...
        let out1 = dir.path().join("a.xelian");
        let out2 = dir.path().join("b.xelian");
    ...
    fn built_archive_is_inspectable_by_tar_and_contains_expected_entries() {
        ...
        let files = collect_files(dir.path()).unwrap();
        let out = dir.path().join("pkg.xelian");
