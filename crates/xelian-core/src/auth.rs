//! Credential management for the Xelian registry (SPEC.md §14.4, §13.7, §13.8).
//!
//! This module handles reading, writing, and deleting the stored registry
//! credential in `~/.xelian/credentials.toml`, which is a top-level cache-
//! adjacent file that MUST survive `xelian rm --all` (§11.3).
...
pub enum CredentialError {
    #[error("not logged in — run `xelian login` first")]
    NotLoggedIn,
...
/// Read the stored credential from `~/.xelian/credentials.toml`.
///
/// Returns `None` if the file does not exist or is unreadable.
pub fn read_credentials(home: &XelianHome) -> Result<Option<StoredCredentials>, CredentialError> {
...
/// Write the credential to `~/.xelian/credentials.toml` with `0600`
/// permissions (Unix) or equivalent restrictive permissions (other platforms).
///
/// Creates the parent `~/.xelian/` directory if it does not exist.
...
/// Remove the stored credential from `~/.xelian/credentials.toml`.
///
/// Does nothing if the file does not exist.
pub fn delete_credentials(home: &XelianHome) -> Result<(), CredentialError> {
    let path = home.credentials_path();
    if path.is_file() {
        fs::remove_file(&path)?;
    }
...
mod tests {
    ...
    /// Serialize tests that read/write `XELIAN_REGISTRY_URL` so they do not
    /// race on the process-global env var.
    static ENV_LOCK: Mutex<()> = Mutex::new(());

    fn test_home() -> (XelianHome, tempfile::TempDir) {
    ...
    fn resolve_url_falls_back_to_env_var() {
        ...
        let (home, _tmp) = test_home();
        std::env::set_var("XELIAN_REGISTRY_URL", "https://custom.example.com");
        let url = resolve_registry_url(&home);
