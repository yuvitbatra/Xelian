//! Integration tests for `harbor add <github-url>` (H-113), exercised via the
...
//!
//! Most of `harbor add`'s behavior (resolve → download → detect → infer →
...
/// Spawn `harbor add <url>` with `HOME` pointed at `home_dir` on the spawned
/// process only (never mutating this test process's own env).
fn run_harbor_add_with_home(url: &str, home_dir: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_harbor"))
        .arg("add")
        .arg(url)
        .env("HOME", home_dir)
        .output()
        .expect("run `harbor add`")
}

#[test]
fn rejects_a_non_github_url_before_any_network_activity() {
    let home_dir = tempfile::tempdir().expect("home dir");
    let output = run_harbor_add_with_home("not-a-github-url", home_dir.path());
    ...
    assert!(!home_dir.path().join(".harbor/packages/github").exists());
}

#[test]
fn rejects_a_non_github_host() {
    let home_dir = tempfile::tempdir().expect("home dir");
    let output = run_harbor_add_with_home("https://gitlab.com/octocat/hello-world", home_dir.path());
...
fn rejects_extra_path_segments_in_the_url() {
    let home_dir = tempfile::tempdir().expect("home dir");
    let output = run_harbor_add_with_home(
...
/// Full end-to-end `harbor add` against a real, small, public GitHub
/// repository: resolves HEAD, downloads, detects language, infers
/// harbor.toml, builds the package, then runs the shared §9.6+ pipeline.
...
fn imports_and_runs_a_real_public_repository() {
    ...
    // packageable agent.
    let output = run_harbor_add_with_home("https://github.com/octocat/Hello-World", home_dir.path());

    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Resolving"), "stderr:\n{stderr}");
    assert!(stderr.contains("Resolved to commit"), "stderr:\n{stderr}");

    let cache_root = home_dir.path().join(".harbor/packages/github/octocat/Hello-World");
