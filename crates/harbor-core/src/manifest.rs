//! The `harbor.toml` manifest: structs, parsing, and validation (SPEC.md §6).
...
/// The full `harbor.toml` manifest (SPEC.md §6).
///
...
pub struct Manifest {
    ...
    /// Native-ecosystem runtime constraint string. Captured verbatim; Harbor
    ...
    /// `[config]` (§6.3) — opaque, never validated or interpreted by Harbor.
    #[serde(default)]
    pub config: toml::Table,
}

impl Manifest {
    /// Parse a `harbor.toml` document from its string contents.
    ///
