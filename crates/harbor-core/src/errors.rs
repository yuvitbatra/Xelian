...
/// Errors that can occur while parsing `harbor.toml` into a [`crate::manifest::Manifest`].
#[derive(Debug, Error)]
pub enum ManifestError {
    ...
    #[error("failed to parse harbor.toml: {0}")]
...
pub enum ValidationError {
    #[error(
        "unsupported spec-version {found}: this harbor binary supports {supported:?}"
    )]
