...
//! available locally, Harbor auto-installs Ollama (if absent) and pulls the
//! model before launch.
//!
...
fn install_ollama(home: &HarborHome) -> Result<std::path::PathBuf, ModelError> {
...
///
/// Models are cached by Ollama in its own store; Harbor does not duplicate
/// the cache in `~/.harbor/models/` in V1 (the directory exists but is
/// reserved for future use).
pub fn ensure_model(primary_model: Option<&str>, home: &HarborHome) -> Result<(), ModelError> {
