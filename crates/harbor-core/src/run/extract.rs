...
///
/// Returns `Ok(true)` if a concurrent `harbor run` of the same archive won
...
pub fn extract_entries(
    ...
    if let Err(e) = fs::rename(&staging_dir, dest) {
        ...
        // extraction starts. If two `harbor run` invocations race on the
...
mod tests {
    ...
    fn extract_entries_happy_path_writes_files_and_renames_into_dest() {
        ...
        let entries = vec![
            ("harbor.toml".to_string(), b"name = \"pkg\"".to_vec(), 0o644),
    ...
    fn concurrent_rename_race_is_treated_as_a_cache_hit() {
        ...
        let entries = vec![("harbor.toml".to_string(), b"name = \"pkg\"".to_vec(), 0o644)];
        ...
        assert!(!dest.join("harbor.toml").exists());
