# Harbor Implementation Roadmap

> An engineering roadmap for building Harbor V1, derived strictly from `SPEC.md`.
...
├── crates/
│   ├── harbor-cli/            # bin: clap command surface, thin
│   └── harbor-core/           # lib: manifest, lockfile, packaging, cache, run pipeline
├── registry/                  # Python FastAPI backend (§14)
├── sdk/                       # Python SDK, wraps the CLI (§15)
└── website/                   # Next.js read-only browse/search (§14.9)
```

Suggested modules inside `harbor-core` (introduced as the phases reach them):
`manifest`, `lockfile`, `package` (archive build/extract), `cache` (`~/.harbor/`
layout), `run` (pipeline: `resolve`, `extract`, `runtime`, `env`, `model`,
`launch`), `permissions`, `registry_client`, `github`. Keep the CLI crate a thin
dispatcher that parses arguments and calls into `harbor-core`.

---

# Phase 0 — Workspace & CLI skeleton

**Goal.** A single `harbor` binary that parses every V1 subcommand
(§13.1–§13.9), prints usage/help, and returns "not implemented" for each. Create
the `~/.harbor/` directory scaffolding (§11.1).
...
**Deliverables.**
- [ ] Initialize a Cargo workspace with `harbor-cli` (bin) and `harbor-core` (lib).
...
- [ ] Each subcommand returns a clear "not yet implemented" message and a
      non-zero exit code.
- [ ] Add a `cache` module that resolves and (lazily) creates the `~/.harbor/`
      tree: `packages/`, `runtimes/`, `envs/`, `models/`, `logs/`, `tmp/`
      (§11.1). Do **not** create or touch `credentials.toml` yet.
- [ ] Global `--version`/`-V` prints the harbor binary version (needed later for
      `harbor.lock`'s `harbor-version`, §7.2).

**Suggested modules/files.** `crates/harbor-cli/src/main.rs` (arg parsing +
dispatch), `crates/harbor-core/src/cache.rs` (path helpers), `harbor-core/src/lib.rs`.

**Acceptance criteria.** `harbor --help` lists all nine commands with correct
flags; each command exits non-zero with a "not implemented" message; running any
command creates the `~/.harbor/` subdirectories (except `credentials.toml`).

**Demo.** `harbor --help` and `harbor run` (prints "not implemented", exits 1).
...
# Phase 1 — `harbor.toml` parsing & validation

**Goal.** Parse `harbor.toml` into typed Rust structs and validate it against the
...
`spec-version` support (§6.1). This is `harbor push` step 1 (§8.1) and the
`harbor run` re-validation check (§9.6), implemented once, reused by both.
...
**Suggested modules/files.** `harbor-core/src/manifest.rs` (structs + `validate`),
a `harbor-core/src/errors.rs` for typed validation errors.
...
`harbor validate` debug command is optional — but the spec defines no such
...
**Goal.** `harbor init` scaffolds a new package in the current directory: a
`harbor.toml` and a `harbor.lock`, with no network access (§13.1).
...
**Deliverables.**
- [ ] Generate a `harbor.toml` skeleton populated with valid defaults and clearly
...
- [ ] Generate a minimal `harbor.lock` shell (fields filled in properly in
      Phase 3; here it can be a valid skeleton).
- [ ] Do not contact the network or registry (§13.1).
- [ ] Refuse to clobber an existing `harbor.toml` without explicit intent
      (decide + document the prompt/flag behavior — see open questions).

**Suggested modules/files.** `harbor-core/src/init.rs`, `harbor-cli` dispatch.

**Acceptance criteria.** Running `harbor init` in an empty dir produces a
`harbor.toml` that parses and validates under Phase 1 (modulo intentional
placeholders), plus a `harbor.lock` skeleton; no network call occurs.

**Demo.** `mkdir weather && cd weather && harbor init`.

**Tests.** *Integration:* run `harbor init` in a temp dir, assert both files
...
# Phase 3 — `harbor.lock` generation & checksums

**Goal.** Generate a correct `harbor.lock` (§7): all required keys (§7.2),
SHA-256 of the native lockfile when declared (`native-lock-checksum`), and the
package archive checksum (`package-checksum`) computed **excluding `harbor.lock`
itself** (§7.3).

**Why.** Checksums are Harbor's only integrity guarantee in V1 (§20.1), and the
"exclude harbor.lock from its own hash" rule is subtle enough to deserve
...
**Deliverables.**
- [ ] Populate all `harbor.lock` keys from §7.2: `spec-version`,
      `harbor-version`, `package-version`, `generated-at` (ISO 8601 UTC),
...
- [ ] Implement `package-checksum` computation over final archive contents
      **excluding `harbor.lock`** (§7.3); never hash `harbor.lock` itself.
- [ ] Copy `package-version` from the manifest `version` (§7.2).
- [ ] Ensure the checksum is stable/deterministic given identical inputs.

**Suggested modules/files.** `harbor-core/src/lockfile.rs`, a `checksum` helper in
`harbor-core/src/package.rs` (shared with Phase 4 archive build).

**Acceptance criteria.** A generated `harbor.lock` contains every required key
with correct values; recomputing `package-checksum` on the same inputs is stable;
`harbor.lock` is provably excluded from `package-checksum`.

**Demo.** After `harbor init` + adding files, an internal build step (wired fully
in Phase 4) prints the computed checksum; verify with `sha256sum` against the
same byte set.

**Tests.** *Unit:* checksum determinism; the exclusion of `harbor.lock` from its
own hash (mutating `harbor.lock` does not change `package-checksum`). *Unit:*
native-lock-checksum matches an independent `sha256sum`.

**Risks.** The circular-hash trap (§7.3) — accidentally including `harbor.lock`
...
"final archive contents excluding `harbor.lock`" but does not fully pin whether
...
# Phase 4 — Packaging: build the `.harbor` archive

**Goal.** Build a spec-conforming `.harbor` archive: a `tar.gz` (§5.1) with the
...
- [ ] Implement the §8.1 pipeline in order: (1) parse+validate manifest [Phase 1],
      (2) re-validate any existing `harbor.lock`, (3) verify required files exist
      ...
      checksum [Phase 3], (7) generate/update `harbor.lock` [Phase 3], (8) build
      the `tar.gz`.
- [ ] Enforce required files: `harbor.toml`, `harbor.lock`, `README.md`, `LICENSE`
      at archive root (§5.3).
...
**Suggested modules/files.** `harbor-core/src/package.rs` (archive build),
`harbor-core/src/validate.rs` (the §8.1 pipeline), reuse `manifest`/`lockfile`.

**Acceptance criteria.** Given a valid package dir, produces a `weather.harbor`
...
**Demo.** `harbor init` a package, add `README.md`/`LICENSE`/entrypoint, then run
the build (initially exposed as part of `push`'s local half, or a temporary
internal build entry) → `tar -tzf weather.harbor` shows the expected layout.
...
**Goal.** Begin the `harbor run` pipeline against a **local `.harbor` archive**:
...
- [ ] Extract into a package-and-version-scoped dir under `~/.harbor/packages/`
      (§9.5, §11.1); skip re-extraction if the same version already exists (§9.5,
      immutability §19.2).
- [ ] Re-parse and re-validate `harbor.toml` using the Phase 1 validator (§9.6),
...
**Suggested modules/files.** `harbor-core/src/run/mod.rs` (pipeline orchestrator),
`harbor-core/src/run/extract.rs`, reuse `cache`, `manifest`, `package`.

**Acceptance criteria.** Running a locally built `weather.harbor` extracts it
under `~/.harbor/packages/…/<version>/` and re-validates; a tampered archive
...
**Demo.** `harbor run ./weather.harbor` → extracts, validates, and stops with a
...
- [ ] Store managed runtimes under `~/.harbor/runtimes/` (§11.1) where the tool
...
**Suggested modules/files.** `harbor-core/src/run/runtime.rs` (dispatch + trait),
...
**Demo.** On a clean-ish machine, `harbor run ./py-agent.harbor` reaches the
...
under `~/.harbor/envs/` (§9.8, §11.1), keyed on `(name, version)` alone (not a
...
- [ ] Install dependencies via the native manager referenced by `dependencies`
      ...
      manifest+lockfile (§9.8). Never re-declare or re-resolve deps in Harbor.
- [ ] Address environments mirroring the source-based cache layout decided in
      Phase 5 (`envs/registry/…`, `envs/github/…`, `envs/local/…`).

**Suggested modules/files.** `harbor-core/src/run/env.rs`, reuse
`run/runtime.rs`.

**Acceptance criteria.** First run of a package builds an env under
`~/.harbor/envs/…/<version>/` and installs deps via the native manager; second
run reuses it without reinstalling; deleting the env forces a rebuild.

**Demo.** `harbor run ./py-agent.harbor` twice: first is slow (installs), second
...
`~/.harbor/envs/` after a run.

**Risks.** Partial/corrupt environments if install is interrupted — consider a
temp-then-rename into `~/.harbor/tmp/` (§11.1) for atomicity. Assuming a native
...
**Why.** This is the payoff phase — after it, `harbor run ./weather.harbor`
...
**Suggested modules/files.** `harbor-core/src/run/launch.rs` (type dispatch),
...
is busy, Harbor picks another and prints it.

**Demo.** `harbor run ./weather.harbor` → chat REPL. For an MCP package:
`harbor run ./some-mcp.harbor` → stdio server announced (with the actual port if
...
**Suggested modules/files.** `harbor-core/src/permissions.rs`, a small
grant-state store under `~/.harbor/` (decide location — see open questions).
...
**Demo.** `harbor run ./weather.harbor` on a fresh cache prompts
...
**Deliverables.**
- [ ] If `primary-model` is declared and not present in `~/.harbor/models/`
...
**Suggested modules/files.** `harbor-core/src/run/model.rs`.
...
**Demo.** `harbor run ./weather.harbor` where `harbor.toml` sets
...
Deciding whether Harbor's `~/.harbor/models/` is authoritative vs. Ollama's own
...
works offline against a locally built package.* This is Harbor's core idea proven
without a registry.
...
**Goal.** Implement `harbor add <github-url>` (§12): resolve the default branch to
a commit SHA, download at that SHA, detect language by precedence, infer a
`harbor.toml` with placeholders, generate `harbor.lock`, build the package, cache
...
- [ ] Generate `harbor.lock` (§12.2 step 4) and build the `.harbor` internally
      (§12.2 step 5).
- [ ] Cache under `~/.harbor/packages/` addressed by SHA (§12.2 step 6).
- [ ] Run from manifest validation onward (§12.2 step 7, i.e. §9.6+).
- [ ] Do **not** publish anything (§12.3) — import is local-only, distinct from
      push.

**Suggested modules/files.** `harbor-core/src/github.rs` (fetch + SHA resolve +
inference), reuse `package`, `lockfile`, `run`.

**Acceptance criteria.** `harbor add <python-repo-url>` produces a cached package
...
**Demo.** `harbor add https://github.com/owner/some-python-agent` → runs.
...
**Goal.** Implement local cache inspection and removal: `harbor list` (§13.5) and
`harbor rm` with `--env` and `--all` (§13.6), respecting credential isolation
(§11.3).
...
**Deliverables.**
- [ ] `harbor list`: list locally cached packages only — not a registry search
      (§13.5, §22).
- [ ] `harbor rm owner/package`: remove all cached versions of that package but
      leave its environment(s) intact (§13.6).
- [ ] `harbor rm owner/package --env`: also remove its cached environment (§13.6).
- [ ] `harbor rm --all`: remove everything under `packages/`, `envs/`,
      `runtimes/`, `models/` — but **never** `credentials.toml` (§13.6, §11.3).
- [ ] `harbor rm` affects only local cache; never the registry (§13.6).

**Suggested modules/files.** `harbor-core/src/cache.rs` (list/remove helpers),
CLI dispatch.

**Acceptance criteria.** `harbor list` shows cached packages; `harbor rm` removes
the package but keeps its env unless `--env`; `harbor rm --all` clears the four
cache dirs and leaves `credentials.toml` untouched.

**Demo.** `harbor list`, then `harbor rm owner/weather --env`, then
`harbor rm --all`.
...
`~/.harbor/`). Accidentally removing `credentials.toml` on `--all` (§11.3
forbids). `harbor list` needs a stable owner/name/version view — depends on the
...
must exist before `harbor push`/`harbor run`-from-registry can be wired. Building
...
- [ ] Implement the data model: `Account`, `Package` (namespace = owner),
      `Versions[]` with version, archive, checksum, `harbor.lock`, README,
...
- [ ] `POST /packages` — publish a new version (§14.8), with receipt checks: the
      ...
      `harbor.lock` (§14.5), and `(name, version)` is not already published
...
built `.harbor`, `GET` its metadata, `GET` its archive.
...
# Phase 14 — `harbor login` / `harbor logout` + credentials

**Goal.** Implement browser-based OAuth login (§13.7, §14.4), store the credential
in `~/.harbor/credentials.toml` with `0600` permissions (§11.1, §14.4), and
`harbor logout` to remove it (§13.8). Add the corresponding registry auth
route(s) (TODO-15).
...
**Deliverables.**
- [ ] `harbor login`: browser-based OAuth flow (like `gh auth login`/
      `docker login`, §13.7); obtain and store a credential.
- [ ] Write `credentials.toml` at `~/.harbor/` top level with `0600` permissions
      (§11.1, §14.4); never inside cache subdirs (§11.3).
- [ ] `harbor logout`: remove the stored credential (§13.8).
...
**Suggested modules/files.** `harbor-core/src/auth.rs` (login flow + credential
store), `registry/app/auth.py` (token/callback routes).

**Acceptance criteria.** `harbor login` completes a browser flow and writes
`credentials.toml` with `0600`; `harbor logout` removes it; `harbor rm --all`
leaves it intact (verified against Phase 12); an authenticated request is
accepted by the registry.

**Demo.** `harbor login` (browser opens, approve), then `harbor logout`.
...
**Open items intersecting this phase.** TODO-15 — exact `harbor login` route
...
- [ ] Run the full §8.1 validation pipeline locally before any network activity
      (§13.2) — reuse Phase 4.
- [ ] Upload the built `.harbor` (and its `harbor.lock`) to `POST /packages`
...
**Suggested modules/files.** `harbor-core/src/registry_client.rs` (HTTP client),
CLI `push` dispatch, reuse `validate`/`package`/`auth`.

**Acceptance criteria.** `harbor push` on a valid, authenticated package uploads
...
**Demo.** `harbor login` then `harbor push` from a package directory → version
...
(`harbor.lock`) and server recompute (§14.5).
...
# Phase 16 — `harbor run` from the registry
...
- [ ] Syntactically distinguish the three target forms — registry ref,
      GitHub URL, and local `.harbor` path (per the 2026-07-16 decision) —
...
- [ ] Check `~/.harbor/packages/` for the resolved exact version before any
      network request; skip download on hit (§9.3).
- [ ] Download the archive via `GET /download/{owner}/{package}/{version}` when
      absent (§9.3), staging through `~/.harbor/tmp/` (§11.1).
...
**Suggested modules/files.** `harbor-core/src/run/resolve.rs` (target parsing +
registry resolution), reuse `registry_client`, `run`.

**Acceptance criteria.** On a machine with an empty cache, `harbor run
...
**Demo.** `harbor run janedoe/research_assistant` on a fresh machine → REPL (the
...
anywhere with zero setup — is fully working.* This is Harbor V1's central success
...
**Goal.** Implement `harbor yank owner/package --version <v>` and `--undo`
...
**Deliverables.**
- [ ] `harbor yank owner/package --version <v>`: authenticated as owner (§14.4),
...
**Suggested modules/files.** `harbor-core/src/registry_client.rs` (yank call),
`registry/app/` yank route, CLI `yank` dispatch.

**Acceptance criteria.** Yanking the latest version causes `harbor run` to resolve
...
**Demo.** `harbor yank janedoe/research_assistant --version 1.2.0` then
`harbor run janedoe/research_assistant` resolves to the prior version
(Appendix C.4).
...
**Deliverables.**
- [ ] `harbor.install(target)`: perform pipeline steps 1–9 without launching
      (§15.2) — shell out to the CLI's prepare path.
- [ ] `harbor.run(target)`: full pipeline (§9.1); return a handle matching the
      package-type (§15.2).
- [ ] `harbor.agent(target)` / `harbor.mcp(target)`: behave like `run` but raise
...
**Suggested modules/files.** `sdk/harbor/__init__.py` (top-level functions),
`sdk/harbor/handles.py` (agent/mcp handles), `sdk/pyproject.toml`.

**Acceptance criteria.** From Python, `harbor.run("owner/pkg").chat("hi")` returns
a response; `harbor.agent()` on an MCP package raises; `harbor.mcp().expose()`
makes the local server available to an MCP client; `install()` prepares without
launching.

**Demo.** A short Python REPL session: `import harbor; harbor.run("owner/pkg")`.
...
- [ ] Package page rendering README (§14.2) and metadata (description, tags,
      permissions, features from `harbor.toml`, §14.2).
...
flowchart TD
    ...
    P1 --> P2[P2 harbor init]
    P1 --> P3[P3 harbor.lock & checksums]
...
The minimum sequence that must be finished, in order, to prove Harbor's core
idea:
...
- [ ] `~/.harbor/logs/` operation logging for `run`/`push`/`add` (the directory
      already exists in §11.1).
- [ ] Progress indicators for downloads, env builds, model pulls (UX polish
      toward the "Ollama feel").
- [ ] A well-crafted `harbor init` template package (still just the two required
...
- Version pinning for `harbor run` (`owner/package@version`) and, contingent on
...
The smallest sequence of milestones that demonstrates Harbor's core idea
("publish an agent, run it anywhere like `ollama run`"):

1. **P0–P4** — author a package and build a `.harbor` archive (`harbor init`,
   then build). *Proves the standard package format exists and is inspectable
   with `tar -tzf`.*
2. **P5–P8** — `harbor run ./weather.harbor` extracts, provisions a runtime,
   installs deps, and drops into a chat REPL. *Proves the local runtime works
   end to end, offline.*
3. **P13–P16** — `harbor push` then, on a clean machine, `harbor run
...
- **TODO-15 (§14.8, affects P14/P17):** exact routes/schemas for `harbor login`
  (OAuth token/callback) and `harbor yank`/unyank beyond the three ratified
  routes.
...
- **`uv`/`npm` invocation details (§9.7/§10, P6/P7):** exact subcommands, flags,
  where CPython/Node are installed, offline behavior, mapping `~/.harbor/runtimes/`
...
- **MCP "expose locally" vs. stdio vs. `port` (§9.10.2, P8) — RESOLVED
  ...
  remains the process transport. *Residual sub-question:* Harbor-side
  stdio↔HTTP bridge vs. port passthrough — decide before P8. SPEC.md
  §9.10.2/§23 need amendment.
- **`harbor init` defaults/prompting (§13.1, P2):** interactive prompts vs.
...
- **Local-archive run target (§9.2, P5/P16) — RESOLVED 2026-07-16:**
  `harbor run ./x.harbor` is a supported third target form, not a test-only
  affordance. SPEC.md §9.2 needs amendment.
- **`package-checksum` domain (§7.3, P3):** whether it is over the tar byte stream
  (minus the `harbor.lock` entry) or a canonical logical file-digest set; keep
...
- **Ollama store authority (§9.9, P10):** whether `~/.harbor/models/` or Ollama's
